...
import type { HoldingPortfolioData } from "@/lib/super-admin/holding";
...
export function HoldingPortfolioContent({
  ...
  portfolio: HoldingPortfolioData;
}) {
  const { holdingName, organizations } = portfolio;
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Vista portfolio — {organizations.length} organizaciones activas
        </p>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <MetricCard
          ...
          value={formatUsd(portfolio.combinedRevenue)}
        />
        <MetricCard
          title="Usuarios activos"
          value={String(portfolio.totalActiveUsers)}
        />
        <MetricCard
          title="Integraciones activas"
          value={String(portfolio.totalIntegrations)}
        />
      </div>

      {organizations.length === 0 ? (
        <p className="text-sm text-muted-foreground">
          No hay organizaciones activas en el portfolio.
        </p>
      ) : (
