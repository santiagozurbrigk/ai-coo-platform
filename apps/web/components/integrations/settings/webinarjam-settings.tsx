 ...
 */
export function WebinarJamSettings({
  status,
  webinars,
}: {
  ...
  function handleConnect() {
    startTransition(async () => {
      ...
      if (!result.success) {
        push({
          title: "No se pudo conectar WebinarJam",
  ...
  function handleSyncWebinars() {
    startTransition(async () => {
      ...
      if (!result.success) {
        push({
          title: "No se pudieron sincronizar los webinars",
          description: result.error,
        });
        return;
      }
  ...
  function handleSyncRegistrants() {
    startTransition(async () => {
      ...
      if (!result.success) {
        push({
          title: "No se pudieron traer los registrantes",
          description: result.error,
        });
        return;
      }
    ...
    <div className="space-y-4">
      {!status.connected ? (
        <div className="space-y-3">
          <div className="space-y-1.5">
            ...
            <p className="inline-flex items-start gap-1.5 text-xs text-muted-foreground">
              ...
              La API de WebinarJam requiere aprobación previa de su equipo — no
              alcanza con tener cuenta. Sirve la misma key para WebinarJam y
              EverWebinar.
            </p>
          </div>
          <Button
        ...
        <div className="space-y-4">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="space-y-1 text-xs">
              <p className="flex items-center gap-1.5 text-sm font-medium">
                ...
                {status.webinarCount} webinars · {status.registrantCount}{" "}
                registrantes
              </p>
          ...
          {webinars.length > 0 ? (
            <div className="space-y-2">
              <p className="text-xs font-medium">
                Segundo en el que aparece la oferta
              </p>
              <p className="text-xs text-muted-foreground">
                WebinarJam no publica este dato, así que hay que cargarlo. Sin
                él, la medida &ldquo;se quedaron hasta la oferta&rdquo; va a
                decir &ldquo;sin datos&rdquo;.
              </p>
              <div className="space-y-2">
                {webinars.map((webinar) => (
                  <PitchSecondRow
                    key={`${webinar.product}:${webinar.webinarId}`}
                    webinar={webinar}
                  />
                ))}
              </div>
          ...
          <p className="inline-flex items-start gap-1.5 text-xs text-muted-foreground">
            <Info className="mt-0.5 h-3.5 w-3.5 shrink-0" />
            El paso &ldquo;Clicked CTA&rdquo; del embudo Webinar queda sin
            fuente: la API de WebinarJam no expone los clicks al CTA. Lo único
            parecido que da es &ldquo;compró en la sala&rdquo;, que mide otra
            cosa.
          </p>
        </div>
      )}
...
function PitchSecondRow({ webinar }: { webinar: WebinarJamWebinarOption }) {
  ...
  const [value, setValue] = useState(
    webinar.pitchSecond ? String(webinar.pitchSecond) : "",
  );
