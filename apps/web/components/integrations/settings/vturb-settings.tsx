...
export function VTurbSettings({ status }: { status: VTurbStatus }) {
  ...
  function handleSync() {
    startTransition(async () => {
      ...
      if (!result.success) {
        push({
          title: "No se pudieron sincronizar los videos",
        ...
        <div className="space-y-3">
          ...
          {status.playersWithoutPitchTime > 0 ? (
            <p className="inline-flex items-start gap-1.5 text-xs text-amber-600 dark:text-amber-400">
              ...
              {status.playersWithoutPitchTime} de {status.playerCount} videos no
              tienen configurado el{" "}
              <span className="font-medium">pitch time</span> en VTurb. En esos,
              la medida &ldquo;llegaron al CTA&rdquo; va a decir &ldquo;sin
              datos&rdquo;: sin saber en qué segundo está la oferta, el número
              que devuelve VTurb cuenta a casi todos los que abrieron el video.
            </p>
          ) : null}
        </div>
      )}
