 ...
 */
export function GHLOpportunitiesSettings({
  status,
}: {
  status: GHLOpportunitiesStatus;
}) {
  ...
  function handleSync() {
    startTransition(async () => {
      ...
      if (!result.success) {
        push({
          title: "No se pudieron sincronizar los pipelines",
      ...
      push({
        ...
        description: `${result.data.pipelines} pipelines · ${result.data.stages} etapas${
          result.data.skipped > 0
            ? ` · ${result.data.skipped} sin id reconocible`
            : ""
        }`,
        variant: "success",
      });
  ...
  function handleRegenerate() {
    startTransition(async () => {
      ...
      if (!result.success) {
        push({
          title: "No se pudo generar el secreto",
          description: result.error,
        });
        return;
      }
        ...
        <div className="text-xs">
          {status.stageHistorySince ? (
            <p className="text-muted-foreground">
              ...
              . {status.transitionCount} transiciones registradas. Los períodos
              anteriores a esa fecha aparecen como &ldquo;sin datos&rdquo;, no
              como cero.
            </p>
          ) : (
            <p className="inline-flex items-start gap-1.5 text-amber-600 dark:text-amber-400">
              <AlertTriangle className="mt-0.5 h-3.5 w-3.5 shrink-0" />
              Todavía no llegó ningún evento. Hasta que llegue el primero, los
              pasos del embudo que dependan de GoHighLevel van a decir
        ...
        {secretUrl ? (
          <div className="rounded-lg border border-primary/30 bg-primary/5 p-3">
            <p className="text-xs font-medium">
              Pegá esta URL en la acción &ldquo;Webhook&rdquo; de un Workflow de
              GoHighLevel
            </p>
            <p className="mt-1 text-xs text-muted-foreground">
              Usá el trigger &ldquo;Opportunity Stage Changed&rdquo;. Esta URL
              contiene el secreto y no se puede volver a ver.
            </p>
