...
const MODELS = [
  {
    value: "last_click",
    label: "Último click",
    hint: "Atribuye al último anuncio antes de la venta",
  },
  {
    value: "first_click",
    label: "Primer click",
    hint: "Atribuye al anuncio que trajo al lead",
  },
  {
    value: "scientific",
    label: "Científico",
    hint: "Ventana de primera atribución configurable",
  },
] as const;
 ...
 */
export function HyrosSettings({ status }: { status: HyrosStatus }) {
  ...
  function handleSync() {
    startTransition(async () => {
      ...
      if (!result.success) {
        push({
          title: "No se pudieron sincronizar las cuentas",
  ...
  function handleModel(model: (typeof MODELS)[number]["value"]) {
    startTransition(async () => {
      ...
      if (!result.success) {
        push({
          title: "No se pudo cambiar el modelo",
          description: result.error,
        });
        return;
      }
      push({
        ...
        description:
          "Los números del período se recalculan con el modelo nuevo.",
        variant: "success",
      });
      router.refresh();
    });
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              Cambia los números, no la presentación: cada modelo responde una
              pregunta distinta sobre el mismo período.
            </p>
          ...
          {status.adAccounts.length > 0 ? (
            <div className="space-y-2">
              <p className="text-xs font-medium">
                Cuentas que entran en los totales
              </p>
              <div className="space-y-1.5">
            ...
            <p className="inline-flex items-start gap-1.5 text-xs text-amber-600 dark:text-amber-400">
              ...
              Sin cuentas publicitarias sincronizadas no se puede pedir ningún
              reporte: la API de Hyros exige nombrarlas.
            </p>
          )}

          <p className="inline-flex items-start gap-1.5 text-xs text-muted-foreground">
            <Info className="mt-0.5 h-3.5 w-3.5 shrink-0" />
            Los números de Hyros no van a coincidir con los de Meta ni con los
            de la pasarela de pagos, y no es un error: miden cosas distintas.
            Por eso el módulo los muestra por separado y etiquetados.
          </p>
        </div>
      )}
    </div>
