...
export type FathomMeetingType = {
  name: string;
  /** `active` se puede asignar a reuniones nuevas; `inactive` sólo persiste en históricas. */
  status: "active" | "inactive";
  createdAt?: string;
};

/**
 * Lista los tipos de reunión de la organización.
 *
 * ⭐ **Es de sólo lectura.** La API de Fathom no tiene endpoints de escritura
 * para tipos: se crean y se asignan dentro de la aplicación de Fathom, no desde
 * OTC. Acá sólo se leen para armar el mapeo tipo → propósito.
 *
 * ⭐ **La clave es el nombre, no un ID.** El schema no expone identificador
 * (`docs/external-apis/fathom/api-reference/meetings/list-meeting-types.md`), así
 * que si alguien renombra un tipo en Fathom el mapeo guardado queda huérfano.
 * Por eso la UI compara contra esta lista viva y avisa, en vez de dejar de
 * clasificar en silencio.
 *
 * Devuelve `null` si la consulta falla —para poder distinguir "la org no tiene
 * tipos" de "no se pudo preguntar"—, y `[]` si responde sin ninguno.
 */
export async function listFathomMeetingTypes(
  apiKey: string
): Promise<FathomMeetingType[] | null> {
  const collected: FathomMeetingType[] = [];
  let cursor: string | undefined;

  // Tope de páginas: una org no tiene cientos de tipos, y evita un bucle si la
  // API devolviera siempre el mismo cursor.
  for (let page = 0; page < 10; page++) {
    const url = new URL(`${FATHOM_API_BASE}/meeting_types`);
    if (cursor) url.searchParams.set("cursor", cursor);
    ...
    const next = typeof obj.next_cursor === "string" ? obj.next_cursor : null;
