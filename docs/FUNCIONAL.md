...
| F-PLA-03 | El usuario puede recuperar una contraseña olvidada | A medias | `AUTH-CALLBACK-NEXT` | [`areas/plataforma.md` § Fuera de plataforma](./areas/plataforma.md#fuera-de-plataforma) · `components/auth/supabase-login-form.tsx` (el link "¿Olvidaste tu contraseña?" no hace nada; `app/auth/recover` y `app/auth/update-password` existen pero nada manda el mail) | |
| F-PLA-04 | Una cuenta creada por Limitless o por un holding entra con contraseña temporal, que vence a las 24 h, y está obligada a cambiarla en el primer ingreso | Funciona | — | [`arquitectura/auth-organizaciones-y-permisos.md` § Alta de cuentas](./arquitectura/auth-organizaciones-y-permisos.md#alta-de-cuentas) · `app/auth/force-password-change/page.tsx` | |
| F-PLA-05 | Una persona invitada puede aceptar la invitación desde el link del mail y entrar a la organización con el rol asignado | Funciona | `INVITE-ROL-SIN-VALIDAR`, `ROL-MEMBER-SIN-CATALOGO` | [`arquitectura/auth-organizaciones-y-permisos.md` § Alta de cuentas](./arquitectura/auth-organizaciones-y-permisos.md#alta-de-cuentas) · `app/invite/page.tsx` | |
...
| F-PLA-19 | El usuario puede elegir qué avisos por mail recibir (reporte semanal, conversación nueva, turno confirmado, venta cerrada) | No funciona | — | [`areas/plataforma.md` § Ajustes](./areas/plataforma.md#ajustes) · `components/settings/settings-form.tsx` (las preferencias se guardan, pero ningún proceso las lee ni manda esos mails) | |
...
| F-PLA-22 | El founder puede importar su histórico de clientes y ventas desde Excel/CSV, GoHighLevel o ClickUp | Con fallas | `CLIENTES-IMPORT-EXCEL-MONTOS`, `CLICKUP-MONTOS` | [`areas/plataforma.md` § Pantallas y rutas](./areas/plataforma.md#pantallas-y-rutas) · `app/(platform)/integrations/import/page.tsx` | |
...
- **"El super admin crea las cuentas a mano; el founder pide usuarios"** (`docs/archivo/PROJECT_CONSTITUTION.md` § Account creation): **existe distinto**. Hay alta pública desde el login (`SIGNUP-PUBLICO`) y el founder invita a su equipo sin pasar por Limitless.
...
- Pestaña Notificaciones de Ajustes: cuatro interruptores que se guardan y no disparan nada.
...
- Pestaña "Mi Calendly" en Ajustes que aparece sólo si el rol se llama literalmente "closer" (`SETTINGS-CLOSER-POR-NOMBRE`).
...
| F-CLI-03 | El founder puede cargar clientes en bloque desde un CSV («Cargar clientes»; si una fila falla no se carga ninguna) | Funciona | `CLIENTES-SIN-MAIL`, `T-3` | [`areas/clientes.md` § Cómo fluye el dato](./areas/clientes.md#cómo-fluye-el-dato) · `components/clients/import-clients-dialog.tsx` | |
| F-CLI-04 | El founder puede importar clientes desde un Excel (en Integraciones → Importar), sin duplicar por nombre | Con fallas | `CLIENTES-IMPORT-EXCEL-MONTOS`, `CLIENTES-SIN-MAIL`, `T-3`, `CLIENTES-TECHO-1000` | [`areas/clientes.md` § Limitaciones conocidas y deuda](./areas/clientes.md#limitaciones-conocidas-y-deuda) · `app/(platform)/integrations/import/page.tsx` → `app/clients/import-actions.ts` | |
...
| F-VEN-21 | Cada miembro del equipo puede conectar su propia cuenta de Fathom y sus grabaciones llegan solas; las no vinculadas quedan privadas de quien grabó | Sin verificar | `B-FATHOM-NUNCA-PROBADO`, `FATHOM-PRIVACIDAD-LEAD` | [`areas/ventas.md` § Fathom → Llamadas](./areas/ventas.md#fathom--llamadas) · `components/integrations/fathom-member-accounts.tsx` | |
...
| F-MKT-15 | El equipo puede ver los comentarios de todas las cuentas y de cada pieza, responderlos y ocultarlos | Con fallas | `ZERNIO-KEY-GLOBAL` | [`areas/marketing.md` § Resto](./areas/marketing.md#resto) · `app/(platform)/comentarios/page.tsx` | |
...
| F-MKT-20 | El founder puede generar un reporte con IA de los patrones de su contenido analizado | Funciona | `MKT-SALES-CONN-VACIA` | [`areas/marketing.md` § Contenido desde Zernio](./areas/marketing.md#contenido-desde-zernio) · `app/marketing/content/pattern-report-actions.ts` | |
...
| F-EMB-13 | El sistema cuenta los comentarios de Instagram (vía Zernio) como disparadores del embudo DM | Sin verificar | `EMBUDOS-CUENTAS-REALES` | [`areas/embudos.md` § Integraciones externas](./areas/embudos.md#integraciones-externas) · `lib/zernio/triggers.ts` | |
...
- **Estado de salud con semáforo y `diagnoseFunnel()`** (FUNNELS_ARCHITECTURE §10, Fase 2; §6.3 "índice con estado de salud de un vistazo" y "switcher con indicador de salud"). No existe en la pantalla: el cálculo está hecho y tiene tests, pero está en pausa. El índice muestra "N de M pasos con fuente" y el switcher no tiene indicador.
...
- **Snapshots, historia y pulso diario** (FUNNELS_ARCHITECTURE §10, Fase 5; §8 "falta el pulso diario"). No existe. La tabla está creada pero nada escribe en ella.
...
| F-IA-14 | El founder puede cargar notas y PDFs en la base de conocimiento, organizarlos en categorías propias y ver el estado de indexado sin recargar | Funciona | — | [`areas/agente-ia.md` § Base de conocimiento y RAG](./areas/agente-ia.md#base-de-conocimiento-y-rag) · `app/(platform)/business-context/documents/page.tsx` | |
| F-IA-15 | El founder puede importar Google Docs y Google Sheets a la base de conocimiento y volver a sincronizarlos | Funciona | — | [`areas/agente-ia.md` § Base de conocimiento y RAG](./areas/agente-ia.md#base-de-conocimiento-y-rag) · `components/business-context/knowledge-base-page.tsx` | |
...
| F-IA-24 | El sistema analiza cada lunes cómo escribe el founder y usa ese tono como contexto para el agente | Funciona | `INTELIGENCIA-SIN-REINTENTO`, `CRONS-ORGS-INACTIVAS` | [`areas/agente-ia.md` § Inteligencia y reportes ejecutivos (crons)](./areas/agente-ia.md#inteligencia-y-reportes-ejecutivos-crons) · `lib/founder-tone/analyze-tone.ts` | |
...
| F-DIS-02 | El founder puede desconectar Discord | A medias | `DISCORD-PERMISOS` | [`areas/discord.md` § Pantallas y rutas](./areas/discord.md#pantallas-y-rutas) · `app/discord/actions.ts` (`disconnectDiscordIntegrationAction` existe pero ningún botón la llama; la tarjeta no ofrece desconectar) | |
