---
title: "Void payment"
source: "https://docs.whop.com/api-reference/payments/void-payment"
capturado: "2026-08-30"
metodo: "POST"
path: "/payments/{id}/void"
---

# Void payment

> Void a payment that has not yet been settled. Voiding cancels the payment before it is captured by the payment processor.

Required permissions:
 - `payment:manage`
 - `plan:basic:read`
 - `access_pass:basic:read`
 - `member:email:read`
 - `member:basic:read`
 - `member:phone:read`
 - `promo_code:basic:read`
 - `shipment:basic:read`
 - `payment:dispute:read`
 - `payment:resolution_center_case:read`



## OpenAPI

_La definición de este endpoint está en el spec oficial._

> **`POST /payments/{id}/void`** — ver [ENDPOINTS-api-v1-stable.md](../../ENDPOINTS-api-v1-stable.md#post-payments-id-void) · spec: [`openapi/api-v1-stable.json`](../../openapi/api-v1-stable.json)