...
> que hoy usan los clientes) contra los módulos que se construyeron en Limitless.
>
...
**Los Excel son un ritual. Limitless es un estado.**

El Excel te dice *cuándo mirar* y *qué preguntarte*. Limitless te dice *cómo están las
...
## Lo que Limitless hace mejor que el Excel
...
## 🔴 Lo que el Excel tiene y a Limitless le falta
...
número). Limitless guarda de dónde partió (`baseline`) y dónde está hoy, pero **no a
dónde iba**.
...
| # | Pregunta | ¿Limitless puede responderla hoy? |
|---|---|---|
...
**Limitless ya tiene los datos de tres de las cuatro y no las muestra juntas en ningún
...
el guion"*) que se pisa cada mes. Limitless tiene hitos alcanzados, que es otra cosa: un
...
El Excel cierra **toda** llamada con próximos pasos. En Limitless eso existe **sólo para
leads** (`sales_leads.next_action`); las llamadas de entrega no tienen dónde.

### 6. El responsable de cada cliente

El Excel tiene coach/responsable en la Config. Limitless no asigna clientes a personas.

### 7. Dos formas de negocio

El Excel ofrece **1-a-1** (completo, con llamadas) y **simplificado** (mentoría de
volumen, sin llamadas individuales). Limitless asume una sola forma.

---

# 2 · Tracker de WINS

## Lo que Limitless hace mejor
...
## 🔴 Lo que le falta a Limitless

### 1. Los permisos del cliente — **el hueco más grave de los dos archivos**

La Ficha de Caso tiene dos campos que Limitless **no tiene en ninguna forma**:
...
**Esto no es una mejora, es una falta.** Hoy Limitless te deja registrar el resultado de
...
El Excel tiene **Estado de uso**: `Sin usar` / `Usada` / `Reservada`. Limitless tiene
"dónde se usó" (`win_usages`), que es el registro **de los que sí se usaron**.

La diferencia es todo: **el Excel te muestra los que NO usaste todavía.** Limitless no
...
En Limitless, `win_usages` son filas libres: registran lo hecho y no señalan el hueco.
...
de éxito** (*"los 3-4 por trimestre que documentás en serio"*). Limitless los mezcla:
todo es un win.
...
La plantilla tiene una estructura que Limitless reduce a **un campo de texto**
(`achievement`):
...
- **Creencias** — *"qué creía que era verdad y no lo era"*. Es el material del que sale el contenido de creencia rota, y **no tiene lugar en Limitless**.
...
es saber **cuáles están pendientes**. Limitless tiene el adjunto o no lo tiene, sin
señal de que falte.
...
El Excel escribe *"5-10k/mes"* → *"+40k/mes"*. Limitless **exige un número**.
...
# 3 · Dos advertencias del Excel que Limitless debería respetar
...
asistente **carga**. Hoy Limitless hace lo contrario en C0/C1: **sólo el founder puede
...
| 2 | **Pantalla de Revisión semanal** | El ritual que falta entero. Limitless ya tiene 3 de las 4 respuestas |
...
columna tiene una razón escrita y cada hoja dice para qué sirve. Lo que Limitless aporta
